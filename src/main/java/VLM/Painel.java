...
public class Painel {
    ...
    public Painel(List<Ponto3D> pontosPlaca, List<Ponto3D> pontosDoVorticeEsquerdoNoPerfil, List<Ponto3D> pontosDoVorticeDireitoNoPerfil, VorticeFerradura vorticeFerradura, EixoDeCoordenadas3D eixoDeCoordenadasDeRotacao) {
...
//      da placa
        this.pontoUmQuartoDoPainel = pontosPlaca.get(0).multiplicaCoordenadasPorEscalar(3.0)
                ...
                .multiplicaCoordenadasPorEscalar(1.0/8.0);
        this.vetorNormal = new Vetor3D(
                pontosPlaca.get(2).subtraiPonto3D(pontosPlaca.get(3))
        )
            .produtoVetorial(
                    new Vetor3D(
                            pontosPlaca.get(0).subtraiPonto3D(pontosPlaca.get(3))
                    )
            );
        this.vetorNormal.posicao = this.pontoDeControle;
    ...
    public void modificaEsteiraDoVortice(Double deslocamentoDaEsteiraEmUmaVolta, Integer numeroDePontosEsteira) {
        ...
        CoordenadaCilindrica pontoBordoDeFugaAerofolioInterno = this.pontosDoVorticeEsquerdoNoPerfil.get(this.pontosDoVorticeEsquerdoNoPerfil.size() - 1).descrevePontoEmNovoEixoDeCoordenadas(eixoDeCoordenadasDeRotacao).converteParaCoordenadaCilindrica();
        CoordenadaCilindrica pontoBordoDeFugaAerofolioExterno = this.pontosDoVorticeDireitoNoPerfil.get(this.pontosDoVorticeDireitoNoPerfil.size() - 1).descrevePontoEmNovoEixoDeCoordenadas(eixoDeCoordenadasDeRotacao).converteParaCoordenadaCilindrica();

        pontosVorticePernaEsquerda.addAll(new CurvaHelicoidal(pontoBordoDeFugaAerofolioInterno, -deslocamentoDaEsteiraEmUmaVolta, false, numeroDePontosEsteira, 1, eixoDeCoordenadasDeRotacao).descrevePontosEmEixoDeCoordenadasPrincipal().pontos);
        pontosVortivePernaDireita.addAll( new CurvaHelicoidal(pontoBordoDeFugaAerofolioExterno, -deslocamentoDaEsteiraEmUmaVolta, false, numeroDePontosEsteira, 1, eixoDeCoordenadasDeRotacao).descrevePontosEmEixoDeCoordenadasPrincipal().pontos);

        this.vorticeFerradura = new VorticeFerradura(1.0, pontosVorticePernaEsquerda,pontosVortivePernaDireita);
    }

    public static Painel constroiPainelHelice(List<Ponto3D> pontosPlaca, List<Ponto3D> pontosCamberAerofolioInterno, List<Ponto3D> pontosCamberAerofolioExterno, Integer numeroDePontosEsteira, EixoDeCoordenadas3D eixoDeCoordenadasDeRotacao) {
        ...
        CoordenadaCilindrica pontoBordoDeFugaAerofolioInterno = pontosCamberAerofolioInterno.get(pontosCamberAerofolioInterno.size() - 1).descrevePontoEmNovoEixoDeCoordenadas(eixoDeCoordenadasDeRotacao).converteParaCoordenadaCilindrica();
        CoordenadaCilindrica pontoBordoDeFugaAerofolioExterno = pontosCamberAerofolioExterno.get(pontosCamberAerofolioExterno.size() - 1).descrevePontoEmNovoEixoDeCoordenadas(eixoDeCoordenadasDeRotacao).converteParaCoordenadaCilindrica();

        pontosVorticePernaEsquerda.addAll(new CurvaHelicoidal(pontoBordoDeFugaAerofolioInterno, -1.0, false, numeroDePontosEsteira, 1, eixoDeCoordenadasDeRotacao).descrevePontosEmEixoDeCoordenadasPrincipal().pontos);
        pontosVortivePernaDireita.addAll( new CurvaHelicoidal(pontoBordoDeFugaAerofolioExterno, -1.0, false, numeroDePontosEsteira, 1, eixoDeCoordenadasDeRotacao).descrevePontosEmEixoDeCoordenadasPrincipal().pontos);

        return new Painel(pontosPlaca, pontosCamberAerofolioInterno, pontosCamberAerofolioExterno, new VorticeFerradura(1.0, pontosVorticePernaEsquerda,pontosVortivePernaDireita), eixoDeCoordenadasDeRotacao);
    }
